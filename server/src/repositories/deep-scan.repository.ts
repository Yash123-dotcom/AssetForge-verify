...
export async function createDeepScan(reportId: string, scan: DeepScanResult): Promise<void> {
  const { error } = await getSupabase().from('deep_scans').insert({ id: scan.scanId, report_id: reportId, package_file_name: scan.package.fileName, package_size_bytes: scan.package.sizeBytes, total_files: scan.package.totalFiles, total_extracted_bytes: scan.package.totalExtractedBytes, composition: scan.composition, detected: scan.detected, risks: scan.risks, confidence: scan.confidence });
