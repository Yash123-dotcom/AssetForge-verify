...
export async function loadMetricsData(): Promise<{ events: EventRow[]; reports: MetricsReportRow[]; feedback: FeedbackRow[]; usefulnessCount: number }> {
  const client = getSupabase();
  const [eventsResult, reportsResult, feedbackResult, usefulnessResult] = await Promise.all([
  ...
  if (eventsResult.error || reportsResult.error || feedbackResult.error || usefulnessResult.error) throw new PersistenceError('Beta metrics could not be loaded.');
  return { events: eventsResult.data ?? [], reports: reportsResult.data ?? [], feedback: feedbackResult.data ?? [], usefulnessCount: usefulnessResult.count ?? 0 };
}
