...
export const verifyRequestSchema = z.object({
  ...
  asset: z.object({
    ...
    metadata: z.object({
      ...
      source: z.enum(['UNITY_ASSET_STORE', 'UPLOADED_PACKAGE']).optional(),
      metadataSource: z.enum(['URL_ANALYSIS', 'MANUAL', 'USER', 'PACKAGE_SCAN']).optional(),
      fieldConfidence: z.object({
        unityVersion: z.enum(['USER', 'PACKAGE', 'HIGH', 'MEDIUM', 'LOW', 'UNKNOWN']).optional(),
        pipeline: z.enum(['USER', 'PACKAGE', 'HIGH', 'MEDIUM', 'LOW', 'UNKNOWN']).optional(),
        dependencies: z.enum(['USER', 'PACKAGE', 'HIGH', 'MEDIUM', 'LOW', 'UNKNOWN']).optional(),
        shaders: z.enum(['USER', 'PACKAGE', 'HIGH', 'MEDIUM', 'LOW', 'UNKNOWN']).optional(),
      }).strict().optional(),
    }).strict().optional(),
  }).strict(),
}).strict();
