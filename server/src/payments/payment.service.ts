...
export type PaymentEvent =
  | { id: string; type: 'PURCHASE_COMPLETED'; provider: 'whop'; userId: string; checkoutId: string; clientReferenceId: string; paymentId: string; planId: string; packId: string; currency: Currency; subtotal: number; taxAmount: number; amountPaid: number; credits: number }
  | { id: string; type: 'PURCHASE_FAILED'; checkoutId: string }
  | { id: string; type: 'PAYMENT_REFUNDED'; refundId: string; paymentId: string; planId: string; currency: Currency; amount: number };
