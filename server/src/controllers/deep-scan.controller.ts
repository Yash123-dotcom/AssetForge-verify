...
import { runDeepScan } from '../services/deep-scan.service.js';
...
export async function createDeepScan(request: Request, response: Response): Promise<void> {
  ...
  try {
    ...
    const result = await runDeepScan(upload); scanId = result.scanId;
    ...
    const code = error instanceof DeepScanError ? error.code : 'INTERNAL_ERROR';
