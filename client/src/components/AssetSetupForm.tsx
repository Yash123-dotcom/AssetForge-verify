...
export function AssetSetupForm({ value, onChange, showErrors }: Props) {
  ...
  return <section className="form-card">
    ...
    <div className="field-grid">
      ...
      <fieldset className="field"><legend>Uses custom shaders?</legend><div className="segment" role="group"><button type="button" aria-pressed={!value.customShaders} className={!value.customShaders ? 'selected' : ''} onClick={() => onChange({ ...value, customShaders: false })}>No</button><button type="button" aria-pressed={value.customShaders} className={value.customShaders ? 'selected' : ''} onClick={() => onChange({ ...value, customShaders: true })}>Yes</button></div></fieldset>
