...
import type { CreditPack, Currency } from '../types/verify.types';

export function PricingPage() {
  const [packs, setPacks] = useState<CreditPack[]>([]); const [currency, setCurrency] = useState<Currency>('INR'); const [busy, setBusy] = useState<string | null>(null); const [error, setError] = useState('');
  const { user } = useAuth(); const navigate = useNavigate();
  useEffect(() => { trackEvent('pricing_viewed'); void getPricing().then((result) => { setPacks(result.packs); setCurrency(result.defaultCurrency); }).catch((caught) => setError(caught instanceof Error ? caught.message : 'Pricing could not be loaded.')); }, []);
  async function buy(pack: CreditPack) {
    trackEvent('credit_pack_selected', { pack: pack.id, currency });
    if (!user) { navigate('/login', { state: { from: '/pricing' } }); return; }
    setBusy(pack.id); setError('');
    try { const result = await createCheckout(pack.id, currency); window.location.assign(result.url); }
  ...
  return <main className="pricing-page shell"><header><p className="eyebrow">PRICING / CREDITS</p><h1>Go deeper when the asset<br /><span>actually matters.</span></h1><p>Quick Check is free. Deep Scan inspects the Unity package itself for shaders, scripts, dependencies, and compatibility signals.</p></header>
    <div className="currency-switch" role="group" aria-label="Currency"><button className={currency === 'INR' ? 'selected' : ''} onClick={() => setCurrency('INR')}>INR</button><button className={currency === 'USD' ? 'selected' : ''} onClick={() => setCurrency('USD')}>USD</button></div>
    {error && <div className="form-error" role="alert">{error}</div>}
    <section className="pricing-grid"><article className="price-card free"><p className="eyebrow">FREE</p><h2>Quick Check</h2><strong className="price-amount">{currency === 'INR' ? '₹0' : '$0'}</strong><ul>{quick.map((item) => <li key={item}><Check />{item}</li>)}</ul><Link className="price-cta" to="/verify">Try Quick Check</Link></article>{packs.map((pack) => <article className={`price-card ${pack.popular ? 'popular' : ''}`} key={pack.id}>{pack.popular && <span className="popular-label">Most Popular</span>}<p className="eyebrow">DEEP SCAN CREDITS</p><h2>{pack.credits} Deep {pack.credits === 1 ? 'Scan' : 'Scans'}</h2><strong className="price-amount">{pack.prices[currency].formatted}</strong><p>{pack.credits} {pack.credits === 1 ? 'credit' : 'credits'} · {pack.credits} package {pack.credits === 1 ? 'scan' : 'scans'}</p><button className="price-cta" disabled={Boolean(busy)} onClick={() => void buy(pack)}>{busy === pack.id ? <LoaderCircle className="spinner" /> : `Buy ${pack.credits} ${pack.credits === 1 ? 'Credit' : 'Credits'}`}</button></article>)}</section>
