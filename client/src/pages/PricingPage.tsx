...
export function PricingPage() {
  ...
  return <main className="pricing-page shell"><header><p className="eyebrow">PRICING / CREDITS</p><h1>Go deeper when the asset<br /><span>actually matters.</span></h1><p>Quick Check is free. Deep Scan inspects the Unity package itself for shaders, scripts, dependencies, and compatibility signals.</p></header>
    ...
    <section className="pricing-grid"><article className="price-card free"><p className="eyebrow">FREE</p><h2>Quick Check</h2><strong>{currency === 'INR' ? '₹0' : '$0'}</strong><ul>{quick.map((item) => <li key={item}><Check />{item}</li>)}</ul><Link className="price-cta" to="/verify">Try Quick Check</Link></article>{packs.map((pack) => <article className={`price-card ${pack.popular ? 'popular' : ''}`} key={pack.id}>{pack.popular && <span className="popular-label">Most Popular</span>}<p className="eyebrow">DEEP SCAN CREDITS</p><h2>{pack.credits} Deep {pack.credits === 1 ? 'Scan' : 'Scans'}</h2><strong>{pack.prices[currency].formatted}</strong><p>1 credit = 1 Deep Scan</p><button className="price-cta" disabled={Boolean(busy)} onClick={() => void buy(pack)}>{busy === pack.id ? <LoaderCircle className="spinner" /> : 'Buy Deep Scan Credits'}</button></article>)}</section>
