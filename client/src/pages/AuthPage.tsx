...
export function AuthPage({ mode }: { mode: 'LOGIN' | 'SIGNUP' }) {
  ...
  async function sendMagicLink() {
    ...
    setBusy(true); const { error: authError } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: `${window.location.origin}/dashboard` } }); setBusy(false);
  ...
  async function forgotPassword() {
    setError(''); setMessage('');
    if (!supabase || !email) { setError('Enter your email first.'); return; }
    setBusy(true); const { error: authError } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: `${window.location.origin}/account` }); setBusy(false);
